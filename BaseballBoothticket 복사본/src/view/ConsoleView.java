...
public class ConsoleView {
    ...
    // 야구시합 목록 보여주기
    public void displayInfo(TicketStorage ticketStorage) {
        for (int i = 0; i < ticketStorage.getNumTickets(); i++) {
            String matchInfo = ticketStorage.getTicketInfo(i);
    ...
    // ticketStorage에 있는 티켓을 ID로 선택하기
    public int selectTicketId(TicketStorage ticketStorage) {
        int matchId;
        boolean result;
        do {
            matchId = readNumber("추가할 야구경기의 ID를 입력하세요 : ");
            result = ticketStorage.isValidTicket(matchId);
            if (!result)
                System.out.print("잘못된 야구경기 ID입니다.");
        } while (!result);

        return matchId;
    }

    // cart에 있는 티켓을 ID로 선택하기
    public int selectMatchId(Cart cart) {
        int matchId;
        boolean result;
        do {
            matchId = readNumber("야구경기 ID 입력 : ");
    ...
    public void inputCustomerInfo(Customer customer) {
        ...
        System.out.println("이글스파크를 이용하시려면 이름과 전화번호를 입력하세요.");
        System.out.print(">> 이름 : ");
